defmodule Changeban.GameDisplay do
  ...
  def print_headings() do
    IO.write("\n")
    IO.puts("|   AU    |         In Progress         |    C    |               Rejected                |")
    IO.puts("|         |   NC    |   V     |   VP    |    C    |   AU    |   NC    |   VA    |   VP    |")
    IO.puts("|  T   C  |  T   C  |  T   C  |  T   C  |  T   C  |  T   C  |  T   C  |  T   C  |  T   C  |")
  end
  ...
  def test_game() do
    Game.new()
    |> ( & %Game{&1 | items: (for item <- &1.items,
                                  do: %Item{item | state: Enum.random(0..8)})}).()
    |> ( & %Game{&1 | items: (for item <- &1.items,
                                  do: %Item{item | blocked: (0 < item.state && item.state < 4 && Enum.random(0..2) == 0)})}).()
    |> ( & %Game{&1 | items: (for item <- &1.items,
                                  do: %Item{item | owner: (if (0 < item.state),
                                                              do: Enum.random(0..4),
                                                              else: nil )})}).()
  end
